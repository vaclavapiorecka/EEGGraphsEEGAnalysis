function varargout = absolutePower1HzTesting(varargin)
% ABSOLUTEPOWER1HZTESTING MATLAB code for absolutePower1HzTesting.fig
%      ABSOLUTEPOWER1HZTESTING, by itself, creates a new ABSOLUTEPOWER1HZTESTING or raises the existing
%      singleton*.
%
%      H = ABSOLUTEPOWER1HZTESTING returns the handle to a new ABSOLUTEPOWER1HZTESTING or the handle to
%      the existing singleton*.
%
%      ABSOLUTEPOWER1HZTESTING('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ABSOLUTEPOWER1HZTESTING.M with the given input arguments.
%
%      ABSOLUTEPOWER1HZTESTING('Property','Value',...) creates a new ABSOLUTEPOWER1HZTESTING or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before absolutePower1HzTesting_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to absolutePower1HzTesting_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @absolutePower1HzTesting_OpeningFcn, ...
                   'gui_OutputFcn',  @absolutePower1HzTesting_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before absolutePower1HzTesting is made visible.
function absolutePower1HzTesting_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to absolutePower1HzTesting (see VARARGIN)

% Choose default command line output for absolutePower1HzTesting
...
% UIWAIT makes absolutePower1HzTesting wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = absolutePower1HzTesting_OutputFcn(hObject, eventdata, handles) 
...
data01 = createTable(FileName01,PathName01,'AbsolutePower1Hz');       % 
...
% Ukládání souborů (místo a název tabulky)
...
if numOfFiles==size(FileName02,2)
    ...
    errordlg('Je nutné, aby jednotlivé časy měly stejný počet souborů!!!','File Error')
end
...
% 
% % Zapíše jména jednotlivých souborů do tabulky
% for i = 1 : 1 : 4
...
data.(FileName).data04.dataSD  = xlsread(getPath,'80_90','C72:AZ83');
        ...
        dataSD = zeros(12,200);
        data.(FileName).dataSD = dataSD;           % 56 = 4*14 (14 fr. pásem, po 4 časech)
        dataMEAN = zeros(12,200);
        data.(FileName).dataMEAN = dataMEAN;       % 56 = 4*14 (14 fr. pásem, po 4 časech)
...
sizeOfData = size(data.(helping).dataSD,1);                                                            % Počet elektrodových párů pro analýzu
statisticParams = {'Průměr';'SD'};
...
for noe = 1 : 1 : sizeOfData                                                        % Pouze 36 vybraných párů do tabulky. Dva vybrané statistické parametry: průměr, směr. odchylka.
...
for nof = 1 : 1 : 50                % Celkem 14 frekvenčních pásem po 4 časech.
...
% Vytvoření souboru s původními hodnotami. Určené pro statistické
% zhodnocení.
...
if numOfElecs ~= 12
    errordlg('Soubor byl vyexportován v jiném než obvyklém formátu. Zkontrolujte data. ','File Error')
else
    ...
    % Ukládá .mat soubor do stejné složky pro jednu medikaci ve čtyřech časech
...
% Počet jednotlivých měřeních ve skupinách
for noc = 1 : 1 : numOfCycles
