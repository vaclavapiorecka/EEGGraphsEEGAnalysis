function varargout = coherenceTesting(varargin)
% COHERENCETESTING MATLAB code for coherenceTesting.fig
%      COHERENCETESTING, by itself, creates a new COHERENCETESTING or raises the existing
%      singleton*.
%
%      H = COHERENCETESTING returns the handle to a new COHERENCETESTING or the handle to
%      the existing singleton*.
%
%      COHERENCETESTING('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in COHERENCETESTING.M with the given input arguments.
%
%      COHERENCETESTING('Property','Value',...) creates a new COHERENCETESTING or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before coherenceTesting_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to coherenceTesting_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help coherenceTesting

% Last Modified by GUIDE v2.5 14-Jul-2016 14:45:56

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @coherenceTesting_OpeningFcn, ...
                   'gui_OutputFcn',  @coherenceTesting_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before coherenceTesting is made visible.
function coherenceTesting_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to coherenceTesting (see VARARGIN)

% Choose default command line output for coherenceTesting
...
% UIWAIT makes coherenceTesting wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = coherenceTesting_OutputFcn(hObject, eventdata, handles) 
...
data01 = createTable(FileName01,PathName01,'Coherence');       % 
...
data03 = statisticCoherence(data03);
...
data04 = statisticCoherence(data04);
...
if numOfFiles==size(FileName02,2)
    data02 = createTable(FileName02,PathName02,'Coherence');
    assignin('base','data02',data02)
else
...
if numOfFiles==size(FileName03,2)
    data03 = createTable(FileName03,PathName03,'Coherence');
    assignin('base','data03',data03)
else
    errordlg('Je nutné, aby jednotlivé èasy mìly stejný poèet souborù!!!','File Error')
end
...
if numOfFiles==size(FileName04,2)
    data04 = createTable(FileName04,PathName04,'Coherence');
    assignin('base','data04',data04)
else
    errordlg('Je nutné, aby jednotlivé èasy mìly stejný poèet souborù!!!','File Error')
end
...
PathName01 = 'C:\Program Files\NUDZ\EEGAnalyzator\application\';
...
data.(FileName).data01.dataMEAN = xlsread(getPath,'baseline','C3:P68');
data.(FileName).data01.dataSD  = xlsread(getPath,'baseline','C72:P137');

data.(FileName).data02.dataMEAN = xlsread(getPath,'20_30','C3:P68');
data.(FileName).data02.dataSD  = xlsread(getPath,'20_30','C72:P137');

data.(FileName).data03.dataMEAN = xlsread(getPath,'50_60','C3:P68');
data.(FileName).data03.dataSD  = xlsread(getPath,'50_60','C72:P137');

data.(FileName).data04.dataMEAN = xlsread(getPath,'80_90','C3:P68');
data.(FileName).data04.dataSD  = xlsread(getPath,'80_90','C72:P137');



        dataSD = zeros(66,56);
        data.(FileName).dataSD = dataSD;           % 56 = 4*14 (14 fr. pásem, po 4 èasech)
        dataMEAN = zeros(66,56);
        data.(FileName).dataMEAN = dataMEAN;       % 56 = 4*14 (14 fr. pásem, po 4 èasech)
        
        for numOfElecs = 1 : 1 : 66
            for numOfFreq = 1 : 1 : 14
...
sizeOfData = 36;                                                            % Poèet elektrodových párù pro analýzu
...
nameToTableE = cell(2*numOfCycles*36,1);
nameToTableS = cell(2*numOfCycles*36,1);

for noe = 1 : 1 : 36                                                        % Pouze 36 vybraných párù do tabulky. Dva vybrané statistické parametry: prùmìr, smìr. odchylka.
...
for nof = 1 : 1 : 14                % Celkem 14 frekvenèních pásem po 4 èasech.
    columnData1((numOfTimes*(nof-1)+1),:) = namesOfBands(:,nof)';
...
namesOfTimes = {'baseline'; '20-30 min.'; '50-60 min.'; '80-90 min.' };
helpNamesOfElecs = data01.data1.electrodes;

namesOfElecs = cell(sizeData,1);

for i=1:1:sizeData
    namesOfElecs(i)= strcat(helpNamesOfElecs(i,1),'-',helpNamesOfElecs(i,2));
end
